use {
    mcp_blockstore::{CommitMeta as StoreCommitMeta, McpStore},
    mcp_crypto_vc::VectorCommitment,
    mcp_types::{BatchKey, CommitmentRoot, Params},
    mcp_wire::{LeaderBlockPayload, RelayAttestation, RevealShred},
...
impl<V: VectorCommitment, S: McpStore> Reconstructor<V, S> {
    pub fn new(params: Params, store: S) -> Self {
        Self {
            params,
            store,
            _vc: Default::default(),
    ...
    pub fn on_block_finalized(&mut self, payload: &LeaderBlockPayload) -> HashSet<BatchKey> {
        ...
        assert!(
            payload.relay_attestations.len() >= min_relay_attestations,
            "µ-threshold failed"
        );
        ...
        for RelayAttestation { relay, entries, .. } in &payload.relay_attestations {
            for (key, commitment_root, _sigma_q) in entries {
                ...
                per_key_commitment
                    .entry(key.clone())
                    .and_modify(|existing| {
                        assert_eq!(existing.0, commitment_root.0, "conflicting commitment root")
                    })
    ...
    pub fn on_reveal(&mut self, rs: RevealShred) -> bool {
        let Some(meta) = self.store.get_commit_meta(&rs.key) else {
            return false;
        };
        if !meta.finalized || !meta.phi_ok {
            return false;
        }
        let commitment_root = rs
            .opt_commitment_root
            .unwrap_or_else(|| meta.commitment_root.clone());
        ...
        let count = self.store.reveal_insert(
            &rs.key,
            rs.index,
            rs.coded_symbol,
            rs.leaf_randomizer,
    ...
    pub fn reconstruct_bytes(&self, key: &BatchKey) -> Option<Vec<u8>> {
        ...
        if self.store.reveal_count(key) < (meta.min_symbols_to_decode + meta.adversary_tolerance) {
            return None;
        }
