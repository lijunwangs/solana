...
pub struct SolanaStore {
    pub db: Arc<BlockstoreLike>,
}
impl SolanaStore {
    pub fn new(db: Arc<BlockstoreLike>) -> Self {
        Self { db }
    }
    ...
    fn key_meta(key: &BatchKey) -> Vec<u8> {
        Self::key_prefix(key)
    }
    fn key_leaf(key: &BatchKey, index: u32) -> Vec<u8> {
        let mut k = Self::key_prefix(key);
        k.extend_from_slice(&index.to_le_bytes());
        k
    }
    fn key_count(key: &BatchKey) -> Vec<u8> {
        let mut k = Self::key_prefix(key);
        k.extend_from_slice(&[0xFF, 0xFF, 0xFF, 0xFF]);
        k
    }
}
impl McpStore for SolanaStore {
    fn put_commit_meta(&self, key: BatchKey, meta: CommitMeta) {
        ...
        self.db
            .db
            .put_cf(cf, Self::key_meta(&key), bincode::serialize(&ser).unwrap())
            .unwrap();
    }
    fn get_commit_meta(&self, key: &BatchKey) -> Option<CommitMeta> {
        ...
        Some(CommitMeta {
            C: mcp_types::CommitmentRoot(ser.C),
            finalized: ser.finalized,
            phi_ok: ser.phi_ok,
            K: ser.K as usize,
            T: ser.T as usize,
        })
    ...
    fn reveal_insert(
        ...
        if self.db.db.get_cf(cf, &k_leaf).unwrap().is_none() {
            let val = bincode::serialize(&LeafSer {
                index,
                c_i: c,
                r_i: r,
                w_i: w,
            ...
            let old = self
                ...
                .get_cf(cf, &k_cnt)
                .unwrap()
                .map(|v| bincode::deserialize::<u32>(&v).unwrap_or(0))
                .unwrap_or(0);
            let new = old.saturating_add(1);
            self.db
                .db
                .put_cf(cf, &k_cnt, bincode::serialize(&new).unwrap())
                .unwrap();
        }
    ...
    fn iter_reveals(
        ...
        for kv in iter {
            ...
            if &k[k.len() - 4..] == [0xFF, 0xFF, 0xFF, 0xFF] {
                continue;
            }
    ...
    fn reveal_count(&self, key: &BatchKey) -> usize {
        ...
        self.db
            ...
            .get_cf(cf, &k_cnt)
            .unwrap()
            .map(|v| bincode::deserialize::<u32>(&v).unwrap_or(0) as usize)
            .unwrap_or(0)
    }
}
