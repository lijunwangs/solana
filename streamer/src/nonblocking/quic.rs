...
async fn run_server(
    ...
    while !exit.load(Ordering::Relaxed) {
        ...
        let incoming = futures::future::select_all(futures).map(|(fut, _i, _)| {
...
async fn track_streamer_fetch_packet_performance(
    ...
    let mut process_sampled_packets_us_hist = stats.process_sampled_packets_us_hist.lock().await;
                ...
                Ok(mut stream) => {
                    ...
                    tokio::spawn(async move {
                        ...
                        while !stream_exit.load(Ordering::Relaxed) {
                            ...
                            .await;
                            // XXX: here: read_chunk stops returning chunks,
                            // so we don't feed packet_sender
                            if let ConnectionPeerType::Staked = peer_type {
                                if res.is_err() {
                                    log::error!("TIMEOUT");
                                }
                            }
