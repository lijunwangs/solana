...
impl ConnectionTable {
    ...
    fn try_add_connection(
        ...
        debug!(
            "Adding connection for key {:?} port {} peer_type {:?} last_update {} max_connections_per_peer: {max_connections_per_peer} remote: {:?}",
            key, port, peer_type, last_update, connection.as_ref().map(|c| c.remote_address())
        );
