...
impl accounts_db_repl_server::AccountsDbRepl for AccountsDbReplServer {
    ...
    async fn get_diff_between_slots(
        &self,
        request: tonic::Request<ReplicaDiffBetweenSlotsRequest>,
    ) -> Result<tonic::Response<ReplicaDiffBetweenSlotsResponse>, tonic::Status> {
        let server = self.accounts_server.read().unwrap();
        let result = server.get_diff_between_slots(&request.into_inner());
        result.map(tonic::Response::new)
    }
}
